...
fn getHasSymbol(content: []const u8, start: usize, stop: usize, numCols: usize) bool {
    //utils.print(">>>> start: {}, stop: {}", .{ start, stop });
    for (start..stop) |i| {
        //utils.print("\n>>>> {}\n", .{i});
        // left char
        if (i > 0) {
...
pub fn solve1(content: []const u8) !i32 {
    ...
    //utils.print("len: {}, num rows: {}, num cols: {}\n", .{ content.len, numRows, numCols });
    var i = content.len;
    ...
    while (i > 0) : (i -= 1) {
        ...
        if (std.ascii.isDigit(content[pos])) {
            ...
            //utils.print(">>>> {}, pos_start: {}, pos_end: {}, has symbol: {}\n", .{ n, pos, content.len - new_i - 1, hasSymbol });
            if (hasSymbol) result += n;
            i = new_i;
        }
        //utils.print("i: {}, pos: {}, char: {}, isDigit: {}\n", .{ i, pos, content[pos], std.ascii.isDigit(content[pos]) });
    }
